    ...
    for f in faculty:
        ...
        if 'gs_pubs' in f:
            ...
            if len(f['gs_pubs']) > 1500:
                ...
                continue 
            title = np.random.choice(f['gs_pubs'])['title']
        
        words = []
        add_words_from_title(words, title, stop_words, lem)
            
        output = open(os.path.join(args.output_dir, '%s.txt' % tag), 'w')
        output.write(' '.join(words))
        output.close()

        output = open(os.path.join(args.output_dir, '%s.real' % tag), 'w')
        output.write(sanitize(title))
        output.close()
